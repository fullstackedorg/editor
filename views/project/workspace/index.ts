...
async function createTransport(project: Project): Promise<Transport & { destroy: () => void }> {
    ...
    const add = async (projectFilePath: string) => {
        ...
        const runDiagnostics = () => {
            originalRequest("textDocument/diagnostic", {
                ...
                view.editorView.dispatch(setDiagnostics(view.editorView.state, diagnostics.items.map(item => ({
                    from: view.editorView.state.doc.line(item.range.start.line + 1).from + item.range.start.character,
                    to: view.editorView.state.doc.line(item.range.end.line + 1).from + item.range.end.character,
        ...
        client.request = (method: string, params: any) => {
            if (method === "textDocument/completion" && params?.context?.triggerCharacter !== undefined) {
        ...
        const view = createCodeMirrorView({
            ...
            extensions: [
                oneDark,
                client.plugin(filePathUri, "typescriptreact")
            ]
        })
